...
my_comparisons_2 <- list(
  c("3 month", "1 year"))
#Then use it in your ggplot
over_time_plot_2 <- ggplot(
  subset(metadata.rbind, !is.na(allergy.case) & bf_3m_status %in% c("exclusive", "partial")),
  aes(x = factor(Visit, levels = c("3 month", "1 year")),
      y = Erysipelatoclostridium_ramosum + pseudo,
      fill = Visit)
) +
  geom_boxplot(color = "black") +
  geom_point(position = position_jitter(width = 0.2), color = "black", alpha = 0.6) +
  ...
  annotation_logticks(sides = "l") +
  stat_compare_means(comparisons = my_comparisons_2, method = "wilcox.test", label = "p.signif", size = 10) +  # show asterisks
  theme_classic() +
  theme(
    legend.position = "none",
    axis.title.x = element_text(size = 16),
    axis.title.y = element_text(size = 16),
    axis.text.x = element_text(size = 16),  # increased from 14
    axis.text.y = element_text(size = 14)
  ) +
  labs(
    x = "Visit",
...
ggsave('eramosum-over-time-2.svg', plot = over_time_plot_2) # Figure 2B


prevalence_table <- metadata.rbind %>%
  filter(
    !is.na(allergy.case),
    bf_3m_status %in% c("exclusive", "partial")
  ) %>%
  group_by(Visit_allergy, allergy.case) %>%
  summarise(
    n_samples = n(),
...
ggsave('eramosum-over-time.svg', plot = over_time_plot) # Figure 2B
